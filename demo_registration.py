...
class MiniDataset(Dataset):
    def __init__(self, files, voxel_size):
...
def execute_icp(src_cloud, tgt_cloud, distance_threshold, initial_transform):
    result = open3d.registration.registration_icp(
        ...
        criteria = open3d.registration.ICPConvergenceCriteria(
            relative_fitness = 1.0e-06,
            relative_rmse = 1.0e-06,
            max_iteration = 30))
    return result
...
if __name__ == '__main__':
    ...
    receivedMesh = open3d.io.read_triangle_mesh("data_ar_localization/mesh_ascii_0.ply")
    verticesNum = len(receivedMesh.vertices)
    receivedCloud = receivedMesh.sample_points_uniformly(number_of_points=(verticesNum*15))
    ...
    dataset = MiniDataset(files=point_cloud_files, voxel_size=0.1)
    ...
    src_pcd = open3d.io.read_point_cloud("data_ar_localization/cloud_bin_0.ply")
    src_data = np.load("data_ar_localization/cloud_bin_0.npz")
    ...
    correspondence_set = np.asarray(result_ransac.correspondence_set)

    corresp_no = len(correspondence_set)

    print(result_ransac)

    print("Elapsed time: " + str(elapsed_time) + " s")

    print("Features length: " + str(src_features.num()))

    print("Features dims: " + str(src_features.dimension()))

    print("Keypoints length: " + str(len(src_keypts.points)))

    # np.savetxt("_correspondence_set", correspondence_set, delimiter = " ")

    np.savetxt("resultTransformationMatrix.txt", result_ransac.transformation, delimiter=" ")

    src_set = np.empty((corresp_no, 3))

    tgt_set = np.empty((corresp_no, 3))

    print("Src_set length: " + str(len(src_set)))

    print("Tgt_set length: " + str(len(tgt_set)))

    print("Correspondences length: " + str(corresp_no))

    for i in range(0, corresp_no, 1):
        src_set[i] = src_keypts.points[correspondence_set[i, 0]]
        tgt_set[i] = tgt_keypts.points[correspondence_set[i, 1]]

    np.savetxt("src_set.txt", src_set, delimiter = " ")
    np.savetxt("tgt_set.txt", tgt_set, delimiter = " ")
    ...
    # for i in range(50):
    #     mesh_box = open3d.geometry.TriangleMesh.create_sphere(radius=0.03)
